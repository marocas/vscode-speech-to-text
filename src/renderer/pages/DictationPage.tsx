...
export const DictationPage: React.FC<DictationPageProps> = ({
  ...
  // Listen for cancel-processing from bubble click
  useEffect(() => {
    if (typeof window.api.onCancelProcessing !== 'function') return;
    const unsubscribe = window.api.onCancelProcessing(() => {
      cancelProcessingRef.current = true;
      toast.info('Processing cancelled. Dictation will be saved as cancelled.');
    });
    return () => unsubscribe();
  }, []);
    ...
    try {
      ...
      if (rawText) {
        ...
        if (cancelProcessingRef.current) {
          await window.api.saveDictation(`[CANCELLED] ${rawText}`);
          await loadHistory();
          return;
        }
        ...
        // Check if cancelled after translation/processing
        if (cancelProcessingRef.current) {
          await window.api.saveDictation(`[CANCELLED] ${textForOutput}`);
          await loadHistory();
          return;
        }
